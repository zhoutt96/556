...
int main(int argc, char **argv) {
    ...
    {
        recvfrom(sock, buffer, sizeof(packet), 0, (struct sockaddr *)&addr, &addrlen);
        auto checksum = recv_packet->checksum;
        recv_packet->checksum = 0;
        if (checksum == cksum((unsigned short*)recv_packet, sizeof(*recv_packet)/2))  //modify
        {
            // send a ack back to the client
    ...
    strlcpy(path,recv_packet->data,i+1);
    ...
    while(endFile == 0 || !q.empty()){
        ...
        if(num>0){
            ...
            if (checksum == cksum((unsigned short*)recv_packet, sizeof(*recv_packet)/2)){
                auto offset = (recv_packet->seq_num-1)*sizeof(recv_packet->data);
                auto length = sizeof(recv_packet->data);
